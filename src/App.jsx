          ...
          {tab === "compraDePontos" && (
            <>
              ...
              {(db.compraDePontos || []).length === 0 ? <div className="mk-empty">Nenhuma compra de pontos registrada ainda.</div> : (
                <div className="mk-table-wrap"><table className="mk-table"><thead><tr><th>Programa</th><th>Pontos</th><th>Valor pago</th><th>Pertence a</th><th>Data</th><th></th></tr></thead><tbody>
                  {(db.compraDePontos || []).map((c) => { const conta = accounts.find((a) => a.id === c.programaId); return <tr key={c.id}><td>{conta?.programa || "—"}</td><td>{Number(c.pontos || 0).toLocaleString("pt-BR")}</td><td className="mk-negative">{formatNegativeBRL(c.valorPago)}</td><td><span className="mk-badge">{ownerLabel(conta?.titularId, dependentes)}</span></td><td>{formatDate(c.data)}</td><td><button className="mk-iconbtn" onClick={() => { setEditingCompraDePontos(c); setShowCompraDePontosForm(true); }} title="Editar"><Pencil size={14} /></button><button className="mk-iconbtn" onClick={() => removeCompraDePontos(c.id)} title="Excluir"><Trash2 size={14} /></button></td></tr>; })}
                </tbody></table></div>
              )}
            </>
          )}
          ...
          {tab === "creditosCartao" && (
            <>
              ...
              {(db.creditosCartao || []).length === 0 ? <div className="mk-empty">Nenhum crédito de cartão registrado ainda.</div> : (
                <div className="mk-table-wrap"><table className="mk-table"><thead><tr><th>Programa ou Co-Branded</th><th>Pontos por R$</th><th>Fatura do mês</th><th>Pontos acumulados</th><th>Pertence a</th><th></th></tr></thead><tbody>
                  {(db.creditosCartao || []).map((c) => { const conta = accounts.find((a) => a.id === c.programaId); return <tr key={c.id}><td>{conta?.programa || c.cartao || "—"}</td><td>{Number(c.pontosPorReal || 0).toLocaleString("pt-BR")}</td><td className="mk-negative">{formatNegativeBRL(c.faturaMes)}</td><td>{Number(c.pontosAcumulados || 0).toLocaleString("pt-BR")}</td><td><span className="mk-badge">{ownerLabel(conta?.titularId, dependentes)}</span></td><td><button className="mk-iconbtn" onClick={() => { setEditingCreditCard(c); setShowCreditCardForm(true); }} title="Editar"><Pencil size={14} /></button><button className="mk-iconbtn" onClick={() => removeCreditCard(c.id)} title="Excluir"><Trash2 size={14} /></button></td></tr>; })}
                </tbody></table></div>
              )}
            </>
          )}
